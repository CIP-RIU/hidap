...
suppressMessages("brapi")
suppressWarnings("brapi")
...
library(brapi, warn.conflicts=FALSE)
library(brapps, warn.conflicts=FALSE)
library(agricolae)
...
#library(leaflet)
#library(withr)
library(st4gi,warn.conflicts=FALSE)
library(tibble)
library(knitr)
#library(countrycode)
#library(fbsites)
...
library(spsurvey, warn.conflicts=FALSE)
...
library(fbcheck)
#library(sbformula)
library(pepa, warn.conflicts=FALSE)
library(shinyFiles)
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      fbcheck::fbcheck_ui_sbase(name= "checkFieldbook_sbase"),
...
sv <- function(input, output, session) ({
  ...
  withProgress(message = 'Loading HiDAP', value = 0, {
    ...
    fbcheck::fbcheck_server_sbase(input, output, session, values)
